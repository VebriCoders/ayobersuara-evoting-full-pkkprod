...
class Auth_pemilih extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->post('login-pemilih')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->session->userdata('session_aktif') == "1") {
    ...
    private function _loginPemilih()
    {
        ...
        // $this->user['user'] = $this->db->get_where('tbl_pemilih', ['nomor_pemilih'  => $nomor_pemilih])->row_array();
        // $this->db->where('nomor_pemilih', $this->user['user']['nomor_pemilih']);
        // $this->db->update('tbl_pemilih', ['session_aktif' => "1"]);
            ...
            if ($user['active'] == 1) {
                if ($user['verifikasi_pemilih'] == 1) {
                    if ($user['status_logout'] == 0) { //status seharusnya 0
                        ...
                        $this->db->where('nomor_pemilih', $nomor_pemilih)->update('tbl_pemilih', $sesiaktif);
    ...
    public function logout()
    {
        ...
        $this->db->update('tbl_pemilih', ['session_aktif' => "0"]);
